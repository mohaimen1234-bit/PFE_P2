...
def build_top_equipment_hybrid_prompt(
    ...
    return f"""You are Victoria, a CMMS maintenance assistant.
Security rules:
- Do not reveal passwords, password hashes, tokens, secrets, authentication data, database credentials, or internal permission mappings.
- Do not generate SQL, PHP, shell scripts, or code intended to extract, dump, modify, bypass, or exfiltrate database data.
- If the user asks for restricted data or data-extraction code, refuse briefly and redirect to maintenance-related help.
- Answer in the same language as the user question.
Use the provided data only. If the answer is not supported, say so clearly.

Question:
{question}

Route: HYBRID
SQL intent: top_equipment_by_claims

Context:
{compact_evidence}

Answer in the same language as the user question. Be concise and factual."""
...
Route: HYBRID
SQL intent: overdue_work_orders
...
Route: HYBRID
SQL intent: spare_parts_below_min_stock
...
Route: HYBRID
SQL intent: generic_explain_set
...
- If the user asks for restricted data or data-extraction code, refuse briefly and redirect to maintenance-related help.
- Answer in the same language as the user question.
Use the provided summary of equipment and their related claims/work orders only. If the answer is not supported, say so clearly.
...
Synthesize a helpful answer explaining which equipment are most problematic and what the related issues seem to be based on the provided notes. Answer in the same language as the user question. Be concise and professional."""
...
- If the user asks for restricted data or data-extraction code, refuse briefly and redirect to maintenance-related help.
- Answer in the same language as the user question.
Use the provided summary of overdue work orders and related blocked tasks only.
...
Synthesize a helpful answer explaining the current state of overdue work orders and any visible blockers. Answer in the same language as the user question. Be concise and professional."""
...
def ask(request: AskRequest):
    ...
    if is_restricted_question(request.question):
        ...
        intent = classify_question(request.question)["intent"]
        question_lower = request.question.lower()
